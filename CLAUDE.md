...
- **2026-07-23** — Справочники (`Nomenclature` в `Catalog.jsx`): кнопка «Excel» —
  выгрузка ВСЕЙ номенклатуры по всем трём типам (СИЗ/оборудование/материалы) в один
  файл. `doExport` параллельно грузит `/api/catalog/items` по каждому `item_type`
  из `TYPE_TABS`, строит строки с колонками Тип/Категория/Подкатегория/Наименование/
  Описание/Ед.изм./Пол/Срок службы/Требуется поверка/Период поверки и шлёт в
  существующий `exportExcel` → `/api/export/xlsx`. Кнопка не зависит от текущей
  вкладки/фильтров — всегда полный справочник. Только фронт.
- **2026-07-20** — `InventoryForm`: подкатегория теперь ВСЕГДА автоподтягивается из
  позиции — при открытии (из `catalog_item`) и при выборе наименования
  (`changeCatalogItem` ставит `categoryId/subcategoryId` из выбранной позиции). Плюс
  прошлая правка (сохранение подкатегории на позицию при Save) остаётся. Подтверждено:
  строка списка `/api/inventory` содержит `catalog_item.category_id/subcategory_id`,
  предзаполнение и условие сохранения корректны — «не сохранялось» было из-за
  тестирования ДО ручной пересборки на Amvera. NB напомнить: деплой ручной.
- **2026-07-20** — «Оборудование»/`InventoryForm`: подкатегория из карточки теперь
  РЕАЛЬНО сохраняется на позицию. Раньше «Подкатегория» была лишь фильтром (на
  позиции не сохранялась), а список подкатегорий брался только из уже
  использованных позиций (нельзя назначить ещё не применённую). Теперь:
  `allSubcats` грузится из `/api/catalog/subcategories`, `subcategoryOptions` = все
  ЗАДАННЫЕ подкатегории выбранной категории; при Save, если подкатегория изменена
  В РАМКАХ той же категории позиции (`Number(categoryId)===selectedCatalog.category_id`
  — защита от переклассификации при просмотре), делается
  `PUT /api/catalog/items/{id} {subcategory_id}`. Подпись поля → «Подкатегория
  позиции» + hint «сохраняется на позиции, для всех её единиц». Бэкенд каталога уже
  сохранял (проверено). Только фронт.
...
- **2026-07-20** — Форма «Редактирование позиции», блок «Размещение»: для ВЫДАННЫХ
  ...
  перенос делается через операции «Возврат»/«Перемещение». Причина: `update` делает
  только `setattr` и НЕ меняет статус/`current_employee_id` — указание склада у
  выданного предмета создавало противоречивое состояние (числится и «выдан», и «на
  складе»), не отнимая у сотрудника. Флаг `isIssued`; для склада (in_stock) селект
  как прежде. payload по-прежнему шлёт `current_warehouse_id: null` (у выданного он и
  так null — no-op, порчи нет). Только фронт.
- **2026-07-20** — Форма «Редактирование позиции», доработка: (1) фильтры категории/
  подкатегории теперь НИКОГДА не сбрасывают выбранное наименование — `catalogFiltered`
  всегда добавляет текущую выбранную позицию в список, даже если она не подходит под
  фильтр (у «квадры» нет подкатегории, а фильтр по подкатегории раньше её выкидывал и
  сбрасывал выбор); `changeCategory/changeSubcategory` больше не трогают
  `catalog_item_id`. (2) Убрано поле «Марка / тип» из формы — оно дублирует наименование
  (позицию справочника). Значение `brand_model` в БД сохраняется (в payload, не
  редактируется здесь), в карточке/импортах остаётся. Только фронт.
